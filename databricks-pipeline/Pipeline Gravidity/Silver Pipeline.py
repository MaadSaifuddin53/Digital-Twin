...
spark.sql("DROP TABLE IF EXISTS main.silver.Gravidity")

# COMMAND ----------

# MAGIC %sql
# MAGIC CREATE TABLE IF NOT EXISTS main.silver.Gravidity(
...
bronze_table = "main.bronze.Gravidity"
...
date_cols = ['Date']
for c in date_cols:
    raw_df = raw_df.withColumn(c, to_date(trim(col(c)), "d-MMM-yyyy"))
...
column_mapping = {
    "Date": "date",
    "LocalSampleName": "localSampleName",
    "Gravid": "gravid",
    "Color": "color",
    "Diver": "diver",
}
...
maint_df = maint_df.withColumn("createdBy", lit("GravidityPipeline-silver-layer-job")) \
        .withColumn("createdDateTime", from_utc_timestamp(current_timestamp(), "Asia/Riyadh")) \
       .withColumn("modifiedBy", lit("GravidityPipeline-silver-layer-job")) \
...
maint_df.write.mode("overwrite").saveAsTable("main.silver.Gravidity")
